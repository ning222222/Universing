...
const upload = multer({
  storage: multer.diskStorage({
    ...
    destination(req, file, cb) {
      cb(null, './src/public/uploadvideo/')
    },
...
router.post('/', upload.single('video'), function (req, res, next) {
  ...
  var video= `./uploadvideo/${req.file.filename}`
  var user_nick = decoded.nickname;
  var sql =
    'insert into tbl_univstar(board_title,audition_video,idx,nickname) values(?,?,?,?)'
  var datas = [title, video,user_idx,user_nick]
